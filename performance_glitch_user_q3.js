...
async function saucedemo(){
    ...
    //Log in User//
    const select_user = getRandomNumber(3,3);
    ...
    await driver.findElement(By.xpath("//select[contains(@data-test,'product-sort-container')]")).click();
    await driver.findElement(By.xpath("(//select[contains(@data-test,'product-sort-container')]/option)[2]")).click();
    await driver.sleep(3000);
    

    //add 1st item//
    await driver.findElement(By.xpath("(//div[contains(@class,'pricebar')]/button)[1]")).click();
    const selectedProduct = await driver.findElement(By.xpath("(//div[contains(@class,'inventory_item_name')])[1]")).getText();
    ...
    //Product check//
    //1st product check//
    const actual_Product = await driver.findElement(By.xpath("(//div[contains(@class,'inventory_item_name')])[1]")).getText();
    expect(selectedProduct).to.eql(actual_Product);


    //price check//
    //price 1
    const price = await driver.findElement(By.xpath("(//div[contains(@class,'inventory_item_price')])[1]")).getText();
    const getNum = price.replace("$", "");
    const getPrice = parseFloat(getNum);


    //Item total
    const itemTotal = (getPrice);
